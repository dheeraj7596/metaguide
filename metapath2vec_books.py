from construct_one_graph_books import make_phrases_map, make_authors_map, make_pub_map, detect_phrase
import numpy as np
...
from cocube_utils_beta_books import get_distinct_labels
from coc_data_utils import get_label_term_json, modify_phrases


def get_book_graph_metapaths(df, tokenizer, id_phrase_map):
    index_word = {}
    ...
    pub_index = []
    for i in range(auth_graph_node_count, pub_graph_node_count):
        pub_index.append("pub" + str(i))
    pub_nodes = IndexedArray(np.array([[-1]] * len(pub_index)), index=pub_index)
    ...
    for i, row in df.iterrows():
        ...
        pub = row["publisher"]
        source_nodes_list.append("doc" + str(i))
        target_nodes_list.append("pub" + str(pub_id[pub]))
    ...
    graph = StellarGraph({"doc": doc_nodes, "phrase": phrase_nodes, "author": author_nodes, "pub": pub_nodes}, edges)
    metapaths = [
        ["doc", "phrase", "doc"],
        ["doc", "author", "doc"],
        ["doc", "pub", "doc"],
